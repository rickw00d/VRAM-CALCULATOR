...
覆蓋率 177/177 只證明「key 都填了」,不證明填的**內容**對。以下用「該語系絕不會出現的字」來擋:

| 語系 | 擋什麼 | 為什麼 |
|---|---|---|
| en | 任何 CJK | 漏標 `data-i18n` |
| zh-CN | 繁體專用字形、日文假名 | 繁中複製貼上(OpenCC 直轉也算)、與日文字典互相污染 |
| ja | 繁體專用字形、**簡體專用字形**、無假名 | 與中文字典互相污染 |

- [ ] 字元黑名單**只能收錄該語系確實不用的字**。日文在用 `並為個時長開進過還億網線業種設資執層極優軟選語態準確`,
      誤收會產生大量假警報
- [ ] **不可用「長句純漢字」當日文未翻的啟發式** —— `公式 playbook`、`同時実行` 都是正常日文卻沒有假名。
      改用簡體字形黑名單才精準(P3 踩過)
- [ ] 日文必須含假名:通篇無假名幾乎可斷定是中文沒翻

### 版面必須逐語系測

```powershell
npm run test:visual          # zh-TW
npm run test:visual:all      # 四語系全跑(zh-TW / zh-CN / ja / en)
node test/visual.js <lang>   # 單一語系
```
